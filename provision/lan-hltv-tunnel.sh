#!/bin/bash
# lan-hltv-tunnel.sh — expose a LAN box's HLTV proxies to the internet through a
# public frp relay, so online viewers can spectate even when the box sits behind
# venue NAT with NO inbound port-forward available.
#
...
#       -> installs frpc (on the bulk disk), maps HLTV 27020-24 -> relay 28020-24, starts it.
#
...
UNIT
    ...
    echo ""
    echo "[relay] frps active on :$CTRL_PORT — public HLTV ports $PUBLIC_BASE-$((PUBLIC_BASE+i-1))/udp"
