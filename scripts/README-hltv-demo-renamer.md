...
- **Game host SSH unreachable** — that host's logs aren't read this poll cycle. Resumes when SSH recovers. If down >4h, in-progress matches' windows get abandoned: each flushes the recording that was live at its open (see *Match cancelled or force-reset* below), and anything else at root is left to **the cleanup sweep, which deletes root auto-*.dem at >6h (every 30 min)**, so rescue anything you care about promptly; the cleanup's renamer-active interlock does NOT protect this case (the renamer is up, the game host isn't).
...
- **Match cancelled or force-reset (no `MATCH_WINDOW_CLOSE`)** — the plugin sends CLOSE only at match end, so a cancelled second half or a `.forcereset` leaves the window open. The renamer used to drop such a window at the 4h abandon and the demo was swept unrenamed at 6h; between 2026-09-03 and 2026-09-10 that lost six halves, three of them real play. Now:
  1. When a *different* match opens on the same HLTV port, that OPEN bounds the orphaned window (`Orphaned window` WARNING) and it is renamed through the normal path — except that a candidate whose last write runs past the bound is the later match's recording and is never taken, even as the only candidate. A false start re-generated under a new `match_id` (e.g. `1789004811-DAL1` → `1789005000-DAL1`) therefore claims nothing.
  2. If nothing opens on that port within 4h, the window is force-flushed with only the recording that was live at its open (`Abandoning stale window` WARNING), so idle play recorded after the cancel is not labelled as the match.

  Both name the file with no half marker (`<type>_<match_id>-<ts>-<map>.dem`), which the organizer's `(_h[12])?` accepts. `ktp-soak-verify` flags both lines.
- **Window closed with no candidate file** — logged at WARNING (`closed but no matching auto-* files`). It is the renamer's closest signal to a proxy that was not recording (e.g. up and bound but not connected to its game server). The renamer has no alert path of its own; `ktp-soak-verify` counts these.
- **Rename target already exists** — skipped with warning; no overwrite. Manual operator intervention required.
- **Wedged SSH session (silent, and the one that has actually bitten)** — paramiko's `timeout`/`banner_timeout`/`auth_timeout` cover the handshake only. Past it, a half-open TCP connection blocks the single-threaded poll loop indefinitely at ~0 CPU. On 2026-08-25 this stalled the service for 53h; `systemctl is-active` read `active` the whole time, so `Restart=on-failure` and `OnFailure=` never fired, and every match demo across 08-25/08-26 (23 halves, ~8,400 frags/day of real play) was purged unrenamed by the 6h cleanup sweep. Three guards now cover it: an SSH keepalive, a per-operation SFTP timeout that drops the session rather than reusing a corpse, and a `WatchdogSec=600` heartbeat that lets systemd kill and restart a loop that stops completing cycles. `ktp-data-server-health.sh` independently reports `hltv-demo-renamer=wedged` when `state.json` stops being rewritten.
  ⚠️ **`socket.timeout` subclasses `OSError`, and `IOError is OSError`** — so the timeout handler MUST stay above the `except (FileNotFoundError, IOError)` clause, or a wedged session is filed as "log absent" and the port is skipped silently forever.
- **Active but not reading (the residual gap the wedge fix does NOT cover)** — the poll loop can iterate happily while every SSH session fails. That rewrites `state.json` on schedule, so a `WatchdogSec` heartbeat, an mtime freshness check and the cleanup interlock all read healthy — while no `MATCH_WINDOW` is ever seen and every match demo ages out unrenamed. **Liveness is not reading.** `state.json` therefore carries `last_read_ok`, a per-host stamp of the last poll that got all the way through that host's ports; `ktp-data-server-health.sh` reports `hltv-demo-renamer-read=<REGION>` per silent host, and `ktp-demo-cleanup-auto.sh` refuses to sweep unless every host is fresh.
  ⚠️ **A torn transport raises `EOFError` or `SSHException`, NEITHER of which is an `OSError`** — so neither the timeout clause nor the old broad `IOError` clause caught them, and because `_connect()` returns early while `self._ssh` is set, the dead session was reused on every later poll forever. Conversely `ConnectionResetError` **is** an `OSError` and was being filed as "log absent" and skipped silently. The stat handler is now ordered `socket.timeout` → `FileNotFoundError` → broad, and anything outside the benign case drops the session.
