...
SETTLE_MINUTES = 10
# A match is finished when every half has an end_time and the last one has settled.
FINISHED = """
  m.match_id IN (
...
def check_missing_halves(db: str, since: int, findings: list[str]) -> None:
    ...
    """.format(c=COLL, since=int(since), finished=FINISHED.format(settle=SETTLE_MINUTES)))
...
def check_aggregate_vs_events(db: str, since: int, findings: list[str],
    ...
    rows = q(db, """
        ...
        WHERE f.frags <> IFNULL(s.kills, 0)
          AND {finished}
    """.format(c=COLL, finished=FINISHED.format(settle=SETTLE_MINUTES)) % since)
...
def check_half_ratio(db: str, since: int, findings: list[str]) -> None:
    ...
    rows = q(db, """
        ...
        WHERE m.st > NOW() - INTERVAL %d MINUTE AND h1.k > 60 AND h2.k < h1.k * %s
    """ % (since, HALF_RATIO_FLOOR))
    for mid, k1, k2 in rows:
        findings.append("half 2 far under half 1: %s -- %s then %s kills (%.0f%%)"
                        % (mid, k1, k2, 100.0 * int(k2) / int(k1)))
