...
#
# ram_mb is measured MemTotal, not nominal: it is the denominator for the memory
# percentages, so a wrong value here silently skews the numbers rather than
# failing. Re-measure if a host is rebuilt.
SERVERS = [
    {'host': '74.91.121.9',     'name': 'Atlanta',  'ram_mb': 32055},
    {'host': '74.91.126.55',    'name': 'Dallas',   'ram_mb': 32054},
    {'host': '66.163.114.109',  'name': 'Denver',   'ram_mb': 15964},
    {'host': '74.91.123.64',    'name': 'NewYork',  'ram_mb': 32055},
    {'host': '172.238.176.101', 'name': 'Chicago',  'ram_mb': 7941},
]
...
def collect_all_metrics():
    ...
    for server in SERVERS:
        ...
        try:
            # Key auth, not a password: root@data already holds a key the fleet
            # accepts, so there is no credential to go stale here and none to
            # hardcode. The old password literal was removed with this change.
            ssh.connect(server['host'], username=SSH_USER, timeout=10,
