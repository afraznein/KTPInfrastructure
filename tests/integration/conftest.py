...
#
# So the override must be in place BEFORE task_init_config fires (+1.0s
# after plugin_cfg). The harness boots `+servercfgfile test_server.cfg`,
# which is the ONLY cfg auto-exec'd at map load — server.cfg is not read.
# test_server.cfg carries `exec ktp_hud_test.cfg` (after a dead loopback
# default); _hud_cvars_setup writes that file with the FakeIngest URL.
# A missing target is a harmless no-op, so Tier-1 smoke runs are unaffected.
...
def _hud_cvars_setup(fake_ingest):
    """Write `dod/ktp_hud_test.cfg` pointing `dod_hud_url` + `dod_hud_key`
    at the FakeIngest listener, BEFORE the plugin's `task_init_config`
    builds its persistent curl headers slist.

    A dedicated file rather than server.cfg: the harness boots with
    `+servercfgfile test_server.cfg`, so test_server.cfg is the only cfg
    auto-exec'd — it carries a tolerant `exec ktp_hud_test.cfg` (a no-op
    when the file is absent, e.g. Tier-1 smoke runs). Removed on teardown.
    No-op in external-server mode (no KTP_HLDS_SERVERFILES).
    """
    serverfiles = _serverfiles_path()
    if serverfiles is None:
        # External-server mode: operator's server points the live plugin at
        # the production backend; we don't touch its config. Tests asserting
        # against fake_ingest will see zero POSTs in that mode.
        yield None
        return

    cfg_path = serverfiles / "dod" / "ktp_hud_test.cfg"
    cfg_path.write_text(
        "// AUTO-GENERATED by tests/integration/conftest.py — do not commit.\n"
        "// Exec'd by test_server.cfg; removed on session teardown.\n"
    ...
    # Teardown — remove the generated file.
    try:
