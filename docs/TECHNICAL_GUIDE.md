...
**Last Updated:** 2026-08-15 (version refresh against the post-August-wave fleet; mechanism descriptions last verified against source 2026-07-20 and spot-corrected since)
...
**What this document is:** the engineering reference for the whole stack. It aims to leave a contributor or future maintainer understanding *how* each layer works and *why* it's built this way — the extension-loader mechanism, the hookchain model, the extension-mode lifecycle and its bug class, the async I/O designs, the match-flow state machine, the deploy pipeline, and the fleet platform. Mechanism descriptions were verified against the actual source trees (2026-07-20); version callouts were re-verified against the deployed fleet by binary checksum on 2026-08-15 (engine 3.22.0.930, KTPAMXX core 2.7.27 + DODX module 2.7.28 — the two ship separately and legitimately differ — ReAPI 5.29.0.365-ktp, AmxxCurl 1.3.15-ktp). Version callouts describe the fleet on that date, not forever; the deploy pipeline moves weekly.
...
│  KTPMatchHandler v0.10.161 - Match workflow, pause, OT, score persistence,  │
│                              Discord, HLStatsX                              │
│  KTPHLTVRecorder v1.7.3   - HLTV health checks + demo-rename markers        │
│                             (recording itself is always-on, HLTV-cfg-driven)│
│  KTPCvarChecker v7.32     - Real-time cvar enforcement + Discord grouping   │
│  KTPFileChecker v2.9      - File consistency validation (audit-only)        │
│  KTPAdminAudit v2.7.20    - Menu kick/ban/changemap + timed + central bans  │
│  KTPPracticeMode v1.4.8   - Practice mode with .grenade, noclip, HUD        │
│  KTPGrenadeLoadout v1.0.11 - Custom grenade loadouts per class via INI      │
│  KTPGrenadeDamage v1.0.5  - Grenade damage reduction by configurable %      │
│  KTPScoreTracker v1.1.5   - Verbose capture scoring + per-cap Discord events│
...
│  DODX Module v2.7.28 (built from KTPAMXX; ships separately from the core,   │
...
│  KTPAMXX v2.7.27 - AMX Mod X fork with extension mode + HLStatsX integration│
...
│  - KTPHLStatsX v0.3.5       - HLStatsX daemon with per-half stats + batching│
...
**Repository:** [github.com/afraznein/KTPMatchHandler](https://github.com/afraznein/KTPMatchHandler) — **Version:** 0.10.161 — **License:** MIT
...
|---------------|-----------------------------------------------------------------|----------|--------------------------------|
| Match Handler | [KTPMatchHandler](https://github.com/afraznein/KTPMatchHandler) | 0.10.161 | Match workflow + explicit OT + score persistence + HLStatsX + AC integration |
